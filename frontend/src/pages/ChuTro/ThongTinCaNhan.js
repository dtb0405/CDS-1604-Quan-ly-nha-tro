...
import { FaUser, FaEnvelope, FaPhone, FaHome, FaLock, FaCamera, FaSave } from 'react-icons/fa';
...
const ThongTinCaNhan = () => {
  ...
  const layThongTin = async () => {
    try {
      console.log('📥 Đang lấy thông tin cá nhân từ database...');
      ...
      console.log('📊 Dữ liệu từ database:', userData);
  ...
  const handleSubmitInfo = async () => {
    ...
    try {
      ...
      if (response.data.nguoi_dung) {
        ...
        console.log('🔑 Token trước khi cập nhật store:', token ? 'Có token' : 'Không có token');
        setAuth(response.data.nguoi_dung, token);
        console.log('🔄 Đã cập nhật auth store với:', response.data.nguoi_dung);
        
        // Kiểm tra token sau khi cập nhật
        const newToken = localStorage.getItem('token');
        console.log('🔑 Token sau khi cập nhật store:', newToken ? 'Có token' : 'Không có token');
      }
      
      // Cập nhật form data với dữ liệu mới từ response
      if (response.data.nguoi_dung) {
        const updatedUserData = response.data.nguoi_dung;
        setFormData({
          ho_ten: updatedUserData.ho_ten || '',
          email: updatedUserData.email || '',
          so_dien_thoai: updatedUserData.so_dien_thoai || '',
          dia_chi: updatedUserData.dia_chi || '',
          anh_dai_dien: updatedUserData.anh_dai_dien || ''
        });
        
        // Cập nhật avatar preview nếu có
        if (updatedUserData.anh_dai_dien) {
          setAvatarPreview(`http://localhost:5001${updatedUserData.anh_dai_dien}`);
        }
      }
      ...
      if (error.response?.status === 401) {
        ...
        // Không tự động đăng xuất, để user tự xử lý
  ...
  const handleUploadAvatar = async () => {
    ...
    try {
      ...
      // Cập nhật form data và avatar preview
      if (response.data.nguoi_dung) {
        const updatedUserData = response.data.nguoi_dung;
        setFormData(prev => ({
          ...prev,
  ...
  // Debug: Log formData ngay trước render
  console.log('🖥️ Render component với formData:', formData);
      ...
      <div className="profile-container">
        <div className="profile-sidebar">
          <div className="avatar-section">
            ...
            <p className="user-role">Chủ nhà trọ</p>
              {avatarFile && (
                <button 
              ...
              {!isEditing ? (
                <div className="profile-view-wrapper">
                  <div className="info-row">
                    <span className="label"><FaUser /> Họ và tên:</span>
                    <span className="value">{formData.ho_ten}</span>
                  </div>
                  <div className="info-row">
                  ...
                  <div className="info-row">
                    <span className="label"><FaPhone /> Số điện thoại:</span>
                    <span className="value">{formData.so_dien_thoai}</span>
                  </div>
                  <div className="info-row">
                    <span className="label"><FaHome /> Địa chỉ:</span>
                    <span className="value">{formData.dia_chi}</span>
                  </div>
                  <div className="info-box">
                    <p><strong>Tên đăng nhập:</strong> {nguoiDung?.ten_dang_nhap}</p>
                    <p><strong>Loại tài khoản:</strong> Chủ nhà trọ</p>
                  </div>
                ...
                <div className="profile-form-wrapper">
                  ...
                  <div style={{display: 'flex', gap: 12}}>
                    ...
                    <button type="button" className="btn-cancel" disabled={loading} onClick={() => setIsEditing(false)}>
                      Hủy
                    </button>
                  </div>
                </div>
